...
object Prefs {
    ...
    private const val KEY_MAP         = "keymap" // JSON: usageCode -> action

    fun get(ctx: Context) = ctx.getSharedPreferences(NAME, Context.MODE_PRIVATE)

    var Context.savedAddr: String?
        get() = get(this).getString(KEY_DEVICE_ADDR, null)
        set(v) { get(this).edit().putString(KEY_DEVICE_ADDR, v).apply() }

    var Context.savedName: String?
        get() = get(this).getString(KEY_DEVICE_NAME, null)
        set(v) { get(this).edit().putString(KEY_DEVICE_NAME, v).apply() }

    var Context.autoStart: Boolean
        get() = get(this).getBoolean(KEY_AUTOSTART, true)
        set(v) { get(this).edit().putBoolean(KEY_AUTOSTART, v).apply() }

    fun saveMap(ctx: Context, map: Map<Int, String>) {
        val jo = JSONObject()
        map.forEach { (k,v) -> jo.put(k.toString(), v) }
        get(ctx).edit().putString(KEY_MAP, jo.toString()).apply()
    }

    fun loadMap(ctx: Context): MutableMap<Int,String> {
        val s = get(ctx).getString(KEY_MAP, "{}") ?: "{}"
        val jo = JSONObject(s)
